...
from .db.models import Cowboy, Exercise
from .explorer import Explorer, FileDesc, FileTypeEnum
from .parser import ParsePattern
...
class Processor:
    ...
    def get_leaderboard(self) -> List[Cowboy]:
        ...
        return sorted(
            self.get_cowboys(), key=lambda cowboy: len(cowboy.exercises)
        )[::-1]
    ...
    def update_cowboy(self, cowboy: Cowboy) -> Optional[str]:
        ...
        if prev_cowboy is not None:
            for got_cowboy in self.get_cowboys():
                if (
                    got_cowboy.username == cowboy.username
                    and got_cowboy.model_id != prev_cowboy.model_id
                ):
                    raise Exception(
                        f"Cowboy with username: `{cowboy.username}` already exists"
                    )
    ...
    def _update_cowboys_exercises(self):
        for cowboy in self.get_cowboys():
            ...
            if (
                explorer.dir_file_desc is not None
                and explorer.last_commit != cowboy.last_commit
            ):
...
class Serializer:
    ...
    def from_cowboy_extended(self, cowboy: Cowboy):
        cowboy_str = (
            f"{cowboy.name}: {len(cowboy.exercises)}\n"
            ...
            f"done: {cowboy.done}\n"
            f"exercises: \[{self.from_exercises(cowboy.exercises)}]"  # noqa: W605
        )
        return cowboy_str
    ...
    def from_cowboy(cowboy: Cowboy) -> str:
        return (
            f"{cowboy.name}: {cowboy.done}/{cowboy.must_be_done}\n"
            f"github-repo: [link](https://github.com/{cowboy.username}/{cowboy.repo})"
        )
    ...
    def leaderboard(cowboys: List[Cowboy]) -> str:
        return "\n\n".join(
            [Serializer.from_cowboy(cowboy) for cowboy in cowboys]
        )


def _must_be_done(datetime_str: str):
    must_be_done = (
        (datetime.now() - datetime.fromisoformat(datetime_str)).days / 7 * 5
    )
    return 356 if must_be_done >= 356 else int(must_be_done)


class Deserializer:
    cowboy_keys = {"username", "name", "repo"}
